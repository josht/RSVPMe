    ...
    case xPDOTransport::ACTION_UPGRADE:
        ...
        foreach ($settings as $key) {
            if (isset($options[$key])) {
                $setting = $object->xpdo->getObject('modSystemSetting',array('key' => 'rsvpme.'.$key));
                if ($setting != null) {
                    $setting->set('value',$options[$key]);
                    $setting->save();
                } else {
                    $object->xpdo->log(xPDO::LOG_LEVEL_ERROR, '[RSVPMe] '.$key.' setting could not be found, so the setting could not be changed.');
                }
            }
        }
