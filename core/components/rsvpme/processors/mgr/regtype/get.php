 ...
 */
if (empty($scriptProperties['id'])) return $modx->error->failure($modx->lexicon('rsvpme.regtype_err_ns'));
$regtype = $modx->getObject('RSVPMeRegType',$scriptProperties['id']);
if (!$regtype) return $modx->error->failure($modx->lexicon('rsvpme.regtype_err_nf'));
